...
for line in out:
    ...
    if user is not None and mem is not None and cpu is not None and proc is not None:
        result_list.append([proc.groups()[0], user.group(), float(mem.groups()[0]), float(cpu.groups()[0])])
...
with open(filename, 'w') as f:
    ...
    print(f'Больше всего памяти использует: {max_memory[:20]}')
